...
void Queue_Init( Queue *queue ) {
	if (queue == NULL) // if queue not initialized   - call Queue_Error
	   Queue_Error(QERR_INIT);
	else {
	   for (int i = 0; i < QUEUE_SIZE; i++) { // set all values to *
		  queue->array[i] = '*';  
	   }
	   queue->firstIndex = 0;
	   queue->freeIndex = 0;
...
void Queue_Front( const Queue *queue, char *dataPtr ) { 
	if (Queue_IsEmpty(queue)) //if queue is empty -> call Queue_Error
...
void Queue_Enqueue( Queue *queue, char data ) {
	if (Queue_IsFull(queue)) // if queue is full -> call Queue_Error
