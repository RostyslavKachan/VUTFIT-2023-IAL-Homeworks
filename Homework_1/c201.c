...
void List_Init( List *list ) {
	...
	list->firstElement = NULL;    //setting default values - NULL
...
void List_Dispose( List *list ) {
	ListElementPtr deleteElement = list->firstElement;  //define first element
	while (deleteElement != NULL) { 
		ListElementPtr temp = deleteElement->nextElement; //store a reference to another element
		free(deleteElement);
		deleteElement = temp;
	}
...
void List_InsertFirst( List *list, int data ) {
    ListElementPtr insertElement = (ListElementPtr)malloc(sizeof(*insertElement));  //alokate memory for new element
	if (insertElement == NULL) //not memory -> List_Error
	   List_Error();
...
void List_DeleteFirst( List *list ) {
	if (list->firstElement != NULL) { //if the list not empty = running	
	  ...
	  ListElementPtr deleteFirstElement = list->firstElement; //definition  of the first element
	  list->firstElement = list->firstElement->nextElement; //assigning priority to the next element
	  free(deleteFirstElement);
	}
}
...
void List_DeleteAfter( List *list ) {
	...
	if (list->activeElement != NULL){
		if (list->activeElement->nextElement != NULL) { 	
			ListElementPtr deleteNextElement = list->activeElement->nextElement; 
			list->activeElement->nextElement = deleteNextElement->nextElement;
...
void List_InsertAfter( List *list, int data ) {
	if (list->activeElement != NULL) { 
	   ListElementPtr newElement = (ListElementPtr)malloc(sizeof(*newElement)); //memory allocation 

	   if (newElement == NULL)  //if there is not enough memory - call List_Error
		   List_Error();
...
void List_GetValue( List *list, int *dataPtr ) {
	if (list->activeElement == NULL)  // if list not active - error
		List_Error();
	
	else
	   *dataPtr = list->activeElement->data; // pointer get value active element 
}
...
void List_SetValue(List* list, int data) {
	if (list->activeElement != NULL) //if list is active  - list get value data
